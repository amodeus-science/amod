function [rebalanceQueue, output] = amod_p_mpc_v10(RoadNetwork, RebWeight, Passengers, Flags)
%{
README:
    INPUTS:
...
for t=1:T
    for i=1:N
        for j=1:N
            if (i == j)
                f(r_flow(i,j,t)) = 0.8*RebWeight*TravelTimes(i,j);
            else
...
iflag = norm(cplex_out - round(cplex_out));
if (iflag > 0.5)
    fprintf('Warning: Solution is fractional. Residue is %d \n', iflag)
else
